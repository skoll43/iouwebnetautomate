...
| 2026-04-16 | Added in-app Explainer: full YAML schema reference with 17 documented sections (topology root, variables, global routing, OSPF, EIGRP, BGP, static, VLANs, interfaces, port-channels, SVIs, STP PVST, STP MST, VTP, HSRP, DHCP, NTP, SSH). Categorized sidebar (Structure / Routing / Switching / FHRP / Services / Security), full-text search, per-section field tables with type / required / default / enum values / description, YAML fragment examples, and generated IOS output previews. Modal is toggleable via "Reference" button in the header; closes with Esc or backdrop click. |
